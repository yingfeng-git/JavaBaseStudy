...
public TreeNode buildTree(int[] preorder, int preStart, int preEnd,
    ...
    for (int i = inStart; i <= inEnd; i++){
        if (preorder[preStart] == inorder[i]){
            treeNode.left = buildTree(preorder, preStart+1, i-inStart+preStart, inorder, inStart, i - 1);
            treeNode.right = buildTree(preorder, i-inStart+preStart+1, preEnd, inorder, i + 1, inEnd);
        }
    }
    return treeNode;
}
